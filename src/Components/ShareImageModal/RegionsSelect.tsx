...
import { AWS_REGIONS, DEFAULT_AWS_REGION } from '../../constants';
import {
  ClonesResponseItem,
  ComposeStatus,
...
const generateOptions = (
  ...
  const existingRegions = clones.map((clone) => clone.request.region);
  existingRegions.concat(DEFAULT_AWS_REGION);
  const updatedOptions = options.map((option) => {
    if (existingRegions.includes(option.value)) {
      option.disableRegion = true;
    }
    return option;
  });
...
const RegionsSelect = ({
  ...
  if (!isSuccess || !isSuccessClones) {
    return null;
  }

  const options = generateOptions(clones.data, AWS_REGIONS);
